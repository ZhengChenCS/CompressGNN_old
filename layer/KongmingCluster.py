...
from kongming import kongming_cluster_forward
from kmeans_pytorch import kmeans
from torch_scatter import scatter
import time


class Kongming_Cluster_Function(torch.autograd.Function):
    @staticmethod
    def forward(ctx, input, random_vectors, param_H, is_training):
        ret = kongming_cluster_forward(input, random_vectors, param_H)
        index = ret[0].long()
            ...
            if self.index_cache is False:
                if output is None:
                    if not self.training:
                        output, _, _ = Kongming_Cluster_Function.apply(
                            input, self.random_vectors, self.param_H, self.is_training)
                        self.output = output
                        return output
                    else:
                        output, vertex_index, _ = Kongming_Cluster_Function.apply(
                            input, self.random_vectors, self.param_H, self.is_training)
                        self.output = output
                ...
                if vertex_index is None:
                    if not self.is_training:
                        output, vertex_index, active_bucket = Kongming_Cluster_Function.apply(
                            input, self.random_vectors, self.param_H, self.is_training)
                        self.vertex_index = vertex_index
