...
class SAINTSampler:
    ...
    def __init__(
        ...
        if os.path.exists(graph_fn):
            ...
            norm_dict = np.load(norm_fn, allow_pickle=True).item()
            aggr_norm = norm_dict["aggr_norm"]
            loss_norm = norm_dict["loss_norm"]
        else:
            ...
            self.subgraphs = np.array(self.subgraphs, dtype=object)

            np.save(graph_fn, self.subgraphs, allow_pickle=True)

            t = time.perf_counter()
            aggr_norm, loss_norm = self.__compute_norm__()
            print(f"Normalization time: [{time.perf_counter() - t:.2f}s]")
            norm_dict = {"aggr_norm": aggr_norm, "loss_norm": loss_norm}
            np.save(norm_fn, norm_dict, allow_pickle=True)
