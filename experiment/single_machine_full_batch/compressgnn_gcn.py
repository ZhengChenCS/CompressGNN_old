...
parser.add_argument('--log_path', type=str, default='log/compressgnn_gcn.log', help="log path")
...
logging.basicConfig(level=logging.INFO, format='%(message)s', filename=args.log_path, filemode='w') # include timestamps


def train_runtime(model, data, epochs, device, patience=10, min_delta=0.001, min_epoch=10):
    optimizer = torch.optim.Adam(model.parameters(), lr=0.001)
    model = model.to(device)
    data = data.to(device)
    mask = data.train_mask.to(device)
    test_mask = data.test_mask.to(device)
    vaild_mask = data.vaild_mask.to(device)
    model.train()
    ...
    for epoch in tqdm(range(epochs)):
        ...
        with torch.no_grad():
            # 使用测试集掩码
            pred = model.inference(data.x, data.edge_index)
...
if __name__ == '__main__':
    ...
    model = GCN(in_features=args.num_features, out_features=args.num_classes, param_H=param_H)
