...
parser.add_argument('--deltaH', type=int, default=0, help="delta param_H")
...
if __name__ == '__main__':
    ...
    model = SGC(in_features=args.num_features, out_features=args.num_classes, K=2, param_H=param_H)
    ...
    print("Propagate time:{:.4f}".format(model.propagate_time))
    print("Cluster time:{:.4f}".format(model.cluster_time))
    print("Reconstruct time:{:.4f}".format(model.reconstruct_time))
    print("Transform time:{:.4f}".format(model.transform_time))
    print("ALL time: {:.4f}s".format(train_time))
